...
import { DownloadIcon, ArrowRight, Smartphone, Briefcase, Loader2, Apple } from 'lucide-react';
import { ConnessiaFooter } from '../components/ui/ConnessiaFooter';

export const WelcomeAdminPage: React.FC = () => {
  const { repo } = useData();
  const navigate = useNavigate();
  const [loading, setLoading] = useState(true);
  const [appName, setAppName] = useState('Mi Negocio');
  ...
  useEffect(() => {
    ...
    const loadInfo = async () => {
      try {
        ...
        console.error('Error precargando datos en WelcomeAdminPage:', err);
      } finally {
        setLoading(false);
      }
    };
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem', textAlign: 'center', maxWidth: '600px', margin: '0 auto' }}>
        ...
        <div style={{ width: '120px', height: '120px', borderRadius: '30px', background: '#fff', boxShadow: '0 10px 25px rgba(0,0,0,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '2rem', overflow: 'hidden' }}>
          {logoUrl ? (
            ...
            <Briefcase size={60} color={primaryColor} />
          )}
        </div>

        <h1 style={{ fontSize: '2rem', fontWeight: 'bold', color: '#1e293b', marginBottom: '1rem', lineHeight: '1.2' }}>
          Instala el Panel de <br />
          <span style={{ color: primaryColor }}>{appName} Admin</span>
        </h1>

        <p style={{ color: '#64748b', fontSize: '1.1rem', marginBottom: '3rem', lineHeight: '1.6' }}>
          Gestiona tus citas en tiempo real, controla horarios, servicios y clientes directamente desde tu bolsillo.
        </p>
        ...
        <div style={{ display: 'flex', gap: '1rem', marginBottom: '3rem', justifyContent: 'center', width: '100%' }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            ...
            <span style={{ fontSize: '0.85rem', color: '#475569', fontWeight: '600' }}>Gestión Total</span>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <div style={{ background: '#fff', padding: '0.8rem', borderRadius: '50%', boxShadow: '0 4px 6px rgba(0,0,0,0.05)', marginBottom: '0.5rem' }}>
              <Smartphone size={24} color={primaryColor} />
            </div>
            <span style={{ fontSize: '0.85rem', color: '#475569', fontWeight: '600' }}>Acceso Rápido</span>
          </div>
        </div>
        ...
        {isIOS ? (
          ...
          >
            ...
            {deferredPrompt ? 'Instalar Panel Admin' : 'Panel Admin Listo'}
          </button>
        )}
        ...
        >
          Continuar al panel de gestión <ArrowRight size={18} />
        </button>
